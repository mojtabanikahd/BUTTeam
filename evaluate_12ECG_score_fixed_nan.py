...
def evaluate_12ECG_score(label_directory, output_directory):
    ...
    nans=np.sum(np.isnan(scalar_outputs),1)>0
    
    print('fraction of nans: ' + str(np.mean(nans)))
    
    labels = labels[nans==0, :]
    scalar_outputs = scalar_outputs[nans==0, :]
    binary_outputs = binary_outputs[nans==0, :]>0
    ...
    for equivalent_classes in equivalent_classes_collection:
        for equivalent_class in equivalent_classes:
            if not equivalent_class in classes:
                ...
                labels=np.concatenate((labels,np.zeros((labels.shape[0],1),dtype=bool)),axis=1)
...
def load_outputs(output_files, normal_class, equivalent_classes_collection):
            ...
            for j, l in enumerate(f):
                ...
                if j==1:
                    ...
                    for arr in arrs:
                        ...
                        number = np.nan if arr=='nan' or arr=='-2147483648' else number ###############################################################
                        row.append(number)
    ...
    binary_outputs = np.zeros((num_recordings, num_classes), dtype=np.float32)  #################################################################
    ...
    for equivalent_classes in equivalent_classes_collection:
        for equivalent_class in equivalent_classes:
            if not equivalent_class in classes:
                classes.append(equivalent_class)
                binary_outputs=np.concatenate((binary_outputs,np.zeros((binary_outputs.shape[0],1),dtype=bool)),axis=1)
                scalar_outputs=np.concatenate((scalar_outputs,np.zeros((scalar_outputs.shape[0],1),dtype=bool)),axis=1)
